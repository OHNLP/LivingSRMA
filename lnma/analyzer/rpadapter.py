...
def _dmetar_trans_scrplt(j, params):
    '''
    Transform the dmetar SUCRA data
    '''
    n = len(j['sucraplot']['probs'])
    treats = j['sucraplot']['rows']
    arr = np.array(j['sucraplot']['probs'])
...
def _gemtc_trans_league(arr, params):
    '''
    Transform the gemtc league table data
    '''
    ...
    for item in arr:
        ...
        for _idx, t2 in enumerate(cols):
            # I don't know why, but the idx in R starts from 1
            idx = _idx + 1
            ...
            if t1 == t2:
                ...
                if t2 in item:
                    # which means this result is generated by Rscript
                    tmp = item[t2]
                    ps = tmp.split(' ')
                    val = float(ps[0])
                    ...
                    # which means this result is generated by rpy2
                    # the column names are X1, X2, X3, ... X(n_trts*3)
                    val = item['X%s' % (n_trts + idx)]
                    val_lci = item['X%s' % idx]
                    val_uci = item['X%s' % (2 * n_trts + idx)]
